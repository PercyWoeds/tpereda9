  ...
  def reportes3
   ...
   @caja_name = " "
    case @caja_id
      ...
      when "2"
        @cajas = @company.get_viatico2(@fecha1,@fecha2,2)  
        puts "caja 2"
        puts @caja_id 
        @caja_name = "CAJA CHICA "

      when "3"
         @cajas = @company.get_viatico2(@fecha1,@fecha2,3)  
        @caja_name = "CAJA CHICA "
      when  "4"
        @cajas = @company.get_viatico2(@fecha1,@fecha2,4)  
         @caja_name = "CAJA CHICA "
      when   "5"
        @cajas = @company.get_viaticotbk(@fecha1,@fecha2)  
         @caja_name = "CAJA CHICA "
    ...
    case params[:print]
      when "To PDF" then 
        begin 
         ...
         Prawn::Document.generate "app/pdf_output/rpt_caja_03.pdf", :page_size=>"A4"   do |pdf|
          pdf.font "Helvetica"
          pdf = build_pdf_header_3b(pdf)
          pdf = build_pdf_body_3b(pdf)
          build_pdf_footer_3b(pdf)
          @lcFileName =  "app/pdf_output/rpt_caja_03.pdf"              
         end     
     
          send_file("app/pdf_output/rpt_caja_03.pdf", :type => 'application/pdf', :disposition => 'inline')
...
# reporte completo
 ...
 table_content = ([ [{:image => image_path, :rowspan => 3 }, {:content =>"SISTEMA DE GESTION INTEGRADO ",:rowspan => 2},"CODIGO ","TP-FZ-F-033"], 
          ["VERSION: ","1"], 
          ["Reporte de Pagos realizados por proveedor " +  "#{@caja_name}"  + "#{@fecha1}"+ " AL " + "#{@fecha2}","Pagina: ","1 de 1 "] 
