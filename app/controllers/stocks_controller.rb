...
#----------------------------------------------------------------------------------
  ...
  def rpt_stocks20
    ...
    #@movements = @company.get_stocks_inventarios2(@fecha1,@fecha2,@categoria,@estado)   
      ...
      HardWorkerWorker.perform_async(@fecha1,@fecha2,@categoria,@estado,@local,@current_user.id) 
...
["FECHA","TIPO","SERIE","NUMERO"] ]
  ...
  def rpt_stocks4
    ...
    @fecha1 = params[:fecha1] 
    @fecha2 = params[:fecha2] 
    @producto =params[:ac_item_id]
             

    @movements = @company.get_stocks_inventarios3(@fecha1,@fecha2, @producto)   
...
#----------------------------------------------------------------------------------
# REPORTE DE articulos sin movimiento
#----------------------------------------------------------------------------------

  def build_pdf_header5(pdf)
      pdf.font "Helvetica" , :size => 8
     $lcCli  =  @company.name 
     $lcdir1 = @company.address1+@company.address2+@company.city+@company.state
  ...
  def rpt_stocks5
    ...
    ## separado por destino 
    #@movements = @company.get_stocks_inventarios2(@fecha1,@fecha2,@categoria,@estado)   
    @movements = @company.get_stocks_inventarios30(@fecha1,@fecha2,@categoria,@estado,@local)   

      
    Prawn::Document.generate("app/pdf_output/stocks5.pdf") do |pdf|            
        pdf.font_families.update("Open Sans" => {
          :normal => "app/assets/fonts/OpenSans-Regular.ttf",
          :italic => "app/assets/fonts/OpenSans-Italic.ttf",
        })
        ...
        pdf.font "Open Sans",:size =>6
        pdf = build_pdf_header5(pdf)
        pdf = build_pdf_body5(pdf)
        build_pdf_footer5(pdf)
        $lcFileName =  "app/pdf_output/stocks5.pdf"      
        
    end     

    $lcFileName1=File.expand_path('../../../', __FILE__)+ "/"+$lcFileName    
    send_file("app/pdf_output/stocks5.pdf", :type => 'application/pdf', :disposition => 'inline')
    MovementDetail.delete_all 
  end
...
#----------------------------------------------------------------------------------
# REPORTE DE articulos con stock negativo. 
#----------------------------------------------------------------------------------

  def build_pdf_header6(pdf)
      pdf.font "Helvetica" , :size => 8
     $lcCli  =  @company.name 
     $lcdir1 = @company.address1+@company.address2+@company.city+@company.state
  ...
  def rpt_stocks6
    ...
    ## separado por destino 
    #@movements = @company.get_stocks_inventarios2(@fecha1,@fecha2,@categoria,@estado)   
    @movements = @company.get_stocks_inventarios40(@fecha1,@fecha2,@categoria,@estado,@local)   

      
    Prawn::Document.generate("app/pdf_output/stocks6.pdf") do |pdf|            
        pdf.font_families.update("Open Sans" => {
          :normal => "app/assets/fonts/OpenSans-Regular.ttf",
          :italic => "app/assets/fonts/OpenSans-Italic.ttf",
        })
        ...
        pdf.font "Open Sans",:size =>6
        pdf = build_pdf_header6(pdf)
        pdf = build_pdf_body6(pdf)
        build_pdf_footer6(pdf)
        $lcFileName =  "app/pdf_output/stocks5.pdf"      
        
    end     

    $lcFileName1=File.expand_path('../../../', __FILE__)+ "/"+$lcFileName    
    send_file("app/pdf_output/stocks6.pdf", :type => 'application/pdf', :disposition => 'inline')
    MovementDetail.delete_all 
  end
