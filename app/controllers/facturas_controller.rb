...
def reportes05
  ...
  def export2
    ...
    @facturas  = Factura.where("fecha>=? and fecha<=? and tipo =?","2021-05-01 00:00:00","2021-06-30 23:59:59","1")
     a = ""
...
###++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 def rpt_coti_1
    ...
    @orden_transporte = @company.get_cotiza(@fecha1,@fecha2) 
      ...
      case params[:print]
        when "To PDF" then 
            redirect_to :action => "rpt_coti_1_pdf", :format => "pdf", 
            :fecha1 => params[:fecha1], :fecha2 => params[:fecha2],
            :tiporeporte =>params[:tiporeporte],:location_id => params[:location_id]
 ...
 def rpt_examen01
    ...
    @proyecto_exam  = @company.get_proyecto_exam_empleado_2 
      ...
      case params[:print]
        ...
        when "To Excel" then render xlsx: 'rpt_examen01_xls'
...
###############################################################################################################
 ...
 def rpt_viatico_20
    ...
    if @tiporeporte == "1"
      ...
      @tipo_compro = "0"
        ...
        if @check_cliente == "true"
              @detalle = @company.comprobante_detalle2(@fecha1,@fecha2,"null", @tipo_compro )
          else
              @detalle = @company.comprobante_detalle2(@fecha1,@fecha2,@employee_id, @tipo_compro )
          end 
    case params[:print]
        when "To PDF" then 
          ...
          begin
             ...
             Prawn::Document.generate "app/pdf_output/TP_CM_F_015.pdf" , :page_layout => :landscape ,:page_size=>"A4"  do |pdf|
                  pdf.font "Helvetica"
                  pdf = build_pdf_header_20(pdf)
                  pdf = build_pdf_body_20(pdf)
                  build_pdf_footer_20(pdf)
 ...
 table_content = ([ [{:image => image_path, :rowspan => 3 }, {:content =>"SISTEMA DE GESTION INTEGRADO ",:rowspan => 2},"CODIGO ","NN"], 
          ["VERSION: ","NN"], 
          ["REPORTE DE VIATICOS POR PLACA   - " + "#{@fecha1}"+ " AL " + "#{@fecha2}","Pagina: ","1 de 1 "] 
...
##############################################################################################################
 ...
 def rpt_viatico_30
    ...
    @company=Company.find(1)   
    @employee_id = params[:ubication_id]   
    @fecha1 = params[:fecha1]    
    @fecha2 = params[:fecha2]
      @tiporeporte = params[:fecha6]
    ...
    if @tiporeporte == "1"
      ...
      @tipo_compro = "0"
        ...
        if @check_cliente == "true"
              @detalle = @company.comprobante_detalle3(@fecha1,@fecha2,"null", @tipo_compro )
          else
              @detalle = @company.comprobante_detalle3(@fecha1,@fecha2,@employee_id, @tipo_compro )
          end 
    case params[:print]
        when "To PDF" then 
          ...
          begin
             ...
             Prawn::Document.generate "app/pdf_output/TP_CM_F_015.pdf" , :page_layout => :landscape ,:page_size=>"A4"  do |pdf|
                  pdf.font "Helvetica"
                  pdf = build_pdf_header_30(pdf)
                  pdf = build_pdf_body_30(pdf)
                  build_pdf_footer_30(pdf)
 ...
 table_content = ([ [{:image => image_path, :rowspan => 3 }, {:content =>"SISTEMA DE GESTION INTEGRADO ",:rowspan => 2},"CODIGO ","NN"], 
          ["VERSION: ","NN"], 
          ["REPORTE DE VIATICOS POR DESTINO   - " + "#{@fecha1}"+ " AL " + "#{@fecha2}","Pagina: ","1 de 1 "] 
