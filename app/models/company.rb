...
def get_customer_payments2(moneda,fecha1,fecha2)
   ...
   @facturas = Tempfactura.order(:customer_id,:year_month,:balance ) 
...
def get_customer_payments_detail_value(fecha1,fecha2,value="total")
 ...
 def actualizar_fecha2
    ...
    facturas = Factura.where(["fecha>=? and fecha<=?","2021-04-21 00:00:00","2021-12-31 23:59:59"] )
    for factura in facturas
...
def get_pendientes_day_cliente3(fecha1,fecha2,customer )
    ...
    @facturas = Factura.select("customer_id").where([" balance <> 0  and  company_id = ? AND fecha >= ? and fecha<= ? and customer_id = ?", self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59",customer ]).group(:customer_id)
    #@notas = Factura.where([" balance < 0 and document_id = ?  and  company_id = ? AND fecha >= ? and fecha<= ?", "2",self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59"]).order(:customer_id,:moneda_id,:fecha)
    return @facturas  
...
def get_pendientes_day_customer_detraccion(fecha1,fecha2,cliente)
 ...
 def get_viaticos0(fecha1,fecha2,cajaid)
   viaticos = Viatico.where(["fecha1 >= ? AND fecha1 <= ? and caja_id = ?", "#{fecha1} 00:00:00", "#{fecha2} 23:59:59",cajaid]).order(:code)
   return viaticos 
 end 
...
def get_proyecto_exam_empleado(proyecto_minero_id )
  ...
  @proyecto_exam_empleado = ProyectoexamDetail.select("employee_id,proyecto_exam_id").where(proyecto_exam_id: proyecto_minero_id).group(:employee_id,:proyecto_exam_id)
