...
def get_pendientes_day_customer_detraccion(fecha1,fecha2,cliente)
 ...
 def get_purchases_year_month_value( year,month, value = "total_amount")
  ...
  def get_purchases_day(fecha1,fecha2)
    @purchases = Purchase.where([" company_id = ? AND date2 >= ? and date2 <= ?  and payment_id <> 12 ", self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59" ]).order(:supplier_id,:moneda_id,:date1,:code)    
    return @purchases 
  end
