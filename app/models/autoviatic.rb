class Autoviatic < ActiveRecord::Base
 ...
 belongs_to :location 
  ...
  validates_presence_of :tramit_id, :fecha ,:location_id ,:proyecto_minero_id,
  :employee_id,
  :supplier_id, :tramit_id,:hora_ingreso,:hora_salida 



   def generate_number(serie)
    if Autoviatic.where("cast(substring(code,1,3)  as int) = ?",serie).maximum("cast(substring(code,5,10)  as int)") == nil 
      self.code = serie.to_s.rjust(3, '0') +"-000001"
    else
    self.code = serie.to_s.rjust(3, '0')+"-"+Autoviatic.where("cast(substring(code,1,3)  as int) = ?",serie).maximum("cast(substring(code,5,10)  as int)").next.to_s.rjust(6, '0') 
